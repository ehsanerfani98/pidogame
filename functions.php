	...
	// return $product->price;
	 if ( $product->price > 0 ) {
	   if ( $product->price && isset( $product->regular_price ) ) {
		 ...
		 $to = $product->price;
		 return '<div class="badge badge-danger mx-2 fs-4"><del>'. ( ( is_numeric( $from ) ) ? woocommerce_price( $from ) : $from ) .' </del>  </div><div class="badge badge-success mx-2 fs-6">'.( ( is_numeric( $to ) ) ? woocommerce_price( $to ) : $to ) .'</div>';
	   } else {
		 $to = $product->price;
		 return '<div class="badge badge-success mx-2 fs-6">' . ( ( is_numeric( $to ) ) ? woocommerce_price( $to ) : $to ) . '</div>';
	   }
	} else {
	  return '<div class="badge badge-success fs-6">رایگان</div>';
	}
 }