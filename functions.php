	...
	// return $product->price;
	 if ( $product->price > 0 ) {
	   if ( $product->price && isset( $product->regular_price ) ) {
		 ...
		 $to = $product->price;
		 return '<div class="badge badge-danger mx-2"><del>'. ( ( is_numeric( $from ) ) ? woocommerce_price( $from ) : $from ) .' </del>  </div><div class="live-colst">'.( ( is_numeric( $to ) ) ? woocommerce_price( $to ) : $to ) .'</div>';
	   } else {
		 $to = $product->price;
