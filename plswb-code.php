...
function custom_price_format($price, $product)
{
    ...
    if ($product->is_type('variable')) {
        ...
        $price = '<div class=" fs-5 py-2">' . wc_price($regular_price) . '</div><span>/</span><div class=" fs-5 py-2">' . wc_price($sale_price) . '</div>';
        ...
        if ($regular_price !== $sale_price && $product->is_on_sale()) {
            ...
            $price = '<div class=" fs-5 py-2"><del>' . wc_price($regular_price) . ' </del>  </div><span>/</span><div class="badge badge-success fs-5 py-2">' . wc_price($sale_price) . '</div>';
        } else {
            if ($sale_price == 0) {
                if ($regular_price == 0) {
                    $price = '<div class=" fs-5 py-2">' . 'رایگان' . '</div>';
                    return $price;
                }
                $price = '<div class=" fs-5 py-2">' . wc_price($regular_price) . '</div>';
                return $price;
            }
        }
        $price = '<div class=" fs-5 py-2"><del>' . wc_price($regular_price) . ' </del>  </div><span>/</span><div class="badge badge-success fs-5 py-2">' . wc_price($sale_price) . '</div>';
    ...
    ?>
        ...
        <script>
            jQuery(document).ready(function($) {
                $('shop-table').removeClass(['shop_table', 'order_details', 'fslm-license-keys-table', 'bg-success', 'rounded', 'p-5', 'text-white']).addClass('table table-striped table-rounded border gy-7 gs-7');
            })
        </script>
<?php
    endif;
